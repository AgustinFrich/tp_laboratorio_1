...
static int addNode(LinkedList* this, int nodeIndex, void* pElement)
{
    ...
    auxTam = ll_len(this);
    if(this != NULL && nodeIndex >= 0 && auxTam >= nodeIndex){
...
int ll_add(LinkedList* this, void* pElement)
{
    ...
    if(this != NULL && auxTam >= 0){
		if(!addNode(this, auxTam, pElement)){
			returnAux = 0;
		}
    }
...
int ll_set(LinkedList* this, int index, void* pElement)
{
	Node * auxNodo;
    int returnAux = -1;
    int auxTam = ll_len(this);

    if(this != NULL && index >= 0 && index <= auxTam){
    	if(auxTam != index){
    		auxNodo = getNode(this, index);
...
int ll_remove(LinkedList* this, int index)
{
    ...
    if(this != NULL && index >= 0 && index < auxTam){
		...
		if(!index){
			...
			auxNodo->pNextNode = nodoAEliminar->pNextNode;
		}
...
LinkedList* ll_subList(LinkedList* this, int from, int to)
{
    ...
    if(this != NULL){
    	if(from >= 0 && to > from && to <= ll_len(this)){
    		...
    		if(cloneArray != NULL){
				for(i = from; i <= to; i++){
					...
					if(auxNodo != NULL){
						if(addNode(cloneArray, i, auxNodo->pElement)){
...
/** \brief Crea y retorna una nueva lista con todos los elementos de la lista pasada como parametro
 *
 * \param pList LinkedList* Puntero a la lista
 *
 * \return LinkedList* Retorna  (NULL) Error: si el puntero a la listas es NULL
...
int ll_sort(LinkedList* this, int (*pFunc)(void* ,void*), int order)
{
    ...
    if(this != NULL && (!order || order == 1) && pFunc != NULL){
    	auxTam = ll_len(this);

    	if(auxTam >= 0){
			for(i = 0; i < auxTam - 1; i++){
				for(j = i + 1; j < auxTam; j++){

					aux = pFunc(ll_get(this, i), ll_get(this, j));

					if((order == 1 && aux >= 0) || (order == 0 && aux <= 0)){
